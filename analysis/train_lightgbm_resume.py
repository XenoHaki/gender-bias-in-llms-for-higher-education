...
OUTPUT_DIR = Path("analysis/modeling/lightgbm_resume")
LOAD_BAR_COLOR = "cyan"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LightGBM on downstream resume scoring data.")
    ...
    parser.add_argument("--num-boost-round", type=int, default=1200, help="Max boosting rounds.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Log interval during CV training; 0 disables fold-level eval logs.",
    )
    parser.add_argument(
        "--shap-sample-size",
        type=int,
...
def write_feature_importance(
    path: Path, feature_names: List[str], gain: np.ndarray, split: np.ndarray
) -> None:
    rows = []
        ...
        rows.append(
            {
                ...
                "importance_split": int(split[i]),
            }
        )
    rows.sort(key=lambda x: x["importance_gain"], reverse=True)
    with path.open("w", encoding="utf-8-sig", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=["feature", "importance_gain", "importance_split"])
        writer.writeheader()
...
def write_shap_summary(
    ...
    # contrib shape: [n_rows, n_features]
...
def run_kfold_cv(
    ...
    split = build_kfold_indices(len(y), folds=folds, seed=seed)
    if not split:
        return {}

    fold_rows: List[Dict[str, float]] = []
    for fold_id, (train_idx, valid_idx) in enumerate(
        tqdm(split, desc=f"{folds}-fold CV", unit="fold", colour=CV_BAR_COLOR), start=1
    ):
        ...
        if cv_verbose_eval and cv_verbose_eval > 0:
            callbacks.insert(0, lgb.log_evaluation(cv_verbose_eval))
        ...
        booster = lgb.train(
            ...
            valid_sets=[dva],
            valid_names=[f"fold{fold_id}_valid"],
            callbacks=callbacks,
        )
...
def main() -> None:
    ...
    categorical_indices = list(range(8))
    params = {
    ...
    if args.cv_folds and args.cv_folds > 1:
        ...
        cv_result = run_kfold_cv(
            ...
            cv_verbose_eval=args.cv_verbose_eval,
        )
        ...
        if isinstance(summary, dict) and summary:
            print(
                "[cv] "
                f"MAE={summary.get('mae_mean', float('nan')):.4f}±{summary.get('mae_std', float('nan')):.4f}, "
                f"RMSE={summary.get('rmse_mean', float('nan')):.4f}±{summary.get('rmse_std', float('nan')):.4f}, "
                f"R2={summary.get('r2_mean', float('nan')):.4f}±{summary.get('r2_std', float('nan')):.4f}"
            )

    print("[train] start LightGBM")
    booster = lgb.train(
        params=params,
        train_set=dtrain,
        num_boost_round=args.num_boost_round,
        ...
        callbacks=[
            lgb.log_evaluation(args.verbose_eval),
    ...
    model_path = args.output_dir / "lightgbm_resume_model.txt"
    ...
    write_feature_importance(fi_path, feature_names, gain, split)

    # SHAP contributions via LightGBM built-in pred_contrib (TreeSHAP).
    # Last column is expected value (base value), others are per-feature contributions.
    ...
    contrib_all = booster.predict(x_shap, num_iteration=booster.best_iteration, pred_contrib=True)
    contrib_all = np.asarray(contrib_all)
    feature_contrib = contrib_all[:, :-1]
    expected_value = contrib_all[:, -1]
